...
function initialize() {
    ...
    document.getElementById('sortByOption').addEventListener('change', handleSortByChange);
    document.getElementById('doneList').addEventListener('change', handleDoneListChange);
...
function fetchTodos(todoComplete, sortBy, todoQuery = '', inCharge = false, request = false) {
    ...
    if (todoQuery) url += `&todoQuery=${encodeURIComponent(todoQuery)}`;
    if (inCharge) url += `&inCharge=${inCharge}`;
    if (request) url += `&request=${request}`;
...
function handleMeInChargeClick(e) {
    ...
    const sortBy = document.getElementById('sortByOption').value;
    const todoQuery = document.getElementById('todoSearchQuery').value.trim();
    fetchTodos(document.getElementById('doneList').checked ? 2 : 1, sortBy, todoQuery, true, false);
}
...
function handleRequestClick(e) {
    ...
    const sortBy = document.getElementById('sortByOption').value;
    const todoQuery = document.getElementById('todoSearchQuery').value.trim();
    fetchTodos(document.getElementById('doneList').checked ? 2 : 1, sortBy, todoQuery, false, true);
}
