...
// 공지사항 등록
document.querySelector("#teamBoardInsert").addEventListener("click", () => {
    ...
    const teamBoardTitle = document.getElementById('teamBoardTitle');
    const teamBoardContent = document.getElementById('teamBoardContent');
    ...
    clone.append('teamBoardTitle', teamBoardTitle.value);
    clone.append('teamBoardContent', teamBoardContent.value);
    ...
    if(teamBoardTitle.value.trim().length == 0){
        alert("제목을 작성해주세요.");
        teamBoardTitle.focus();
        return;
    }

    if(teamBoardContent.value == '<p><br></p>' || teamBoardContent.value == '<br>') {
        alert("내용을 작성해주세요.");
    ...
    fetch("/teamBoard/teamBoardInsert" , {
        method : "POST",
        body : clone
    })
    .then(resp => resp.text())
    .then(result => {
        ...
        if(result > 0) {
            ...
            location.href = "/teamBoard/teamBoardDetail/" + result + "?cp=1";
