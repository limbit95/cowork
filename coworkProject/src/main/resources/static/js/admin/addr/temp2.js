    ...
    if(comAddrList != null) {
        ...
        for(let i = 0; i < comAddrList.length; i++) {
            for(let x = 0; x < comAddrList[i].teamList.length; x++) {
                teamList[i].push(
                    {
                        "deptNo" : comAddrList[i].deptNo,
                        "deptNm" : comAddrList[i].deptNm,
                ...
                success : function(result) {
                    ...
                    `
                        <div class="head">
                            ...
                            ${deptList.length > 0 ? `<div><span>부서</span></div>` : `<div style="width: 1px; margin: 0; padding: 0;"></div>`}
                            ${deptList.length > 0 ? `<div><span>팀</span></div>` : `<div style="width: 1px; margin: 0; padding: 0;"></div>`}
                            ${positionList.length > 0 ? `<div><span>직급</span></div>` : `<div style="width: 1px; margin: 0; padding: 0;"></div>`}
                                        ...
                                        `
                                            <div>
                                                <select class="default-line deptList">
                                                    <option class="excelDeptOpt" value="${i.부서 == 'null' ? "부서 선택" : i.부서}" >${i.부서 == 'null' ? "부서 선택" : i.부서}</option>
                                        ...
                                        `
                                            <div>
                                                <select class="default-line teamList">
                                                    <option class="inputTeamOpt" value="${i.부서 == 'null' ? "" : i.팀}">${i.부서 == 'null' ? "" : i.팀}</option>
                                                </select>
                                            </div>
                                        `
                                    :
                                        ...
                                        `
                                            <div>
                                                <select class="default-line positionList">
                                                    <option class="positionOpt" value="${i.직급 == 'null' ? "" : i.직급}">${i.직급 == 'null' ? "" : i.직급}</option>
                    ...
                    document.querySelectorAll(".excelDeptOpt").forEach((x) => {
                        let check = false;
                    ...
                    // if (deptList != null) {
                    //     document.querySelectorAll(".inputTeamOpt").forEach((x) => {
                    //         const deptSelectTag = x.parentElement.parentElement.previousElementSibling.children[0];
                    //         if(deptSelectTag.style.borderColor == 'red') {
                    //             return;
                    //         }
                    //         let flag8 = true;
                    //         comAddrList.forEach((i) => {
                    //             if(i.deptNm == deptSelectTag.children[0].text) {
                    //                 i.teamList.forEach((v) => {
                    //                     if(v.teamNm == x.text) {
                    //                         flag8 = false;
                    //                         return;
                    //                     }
                    ...
                    if(deptList != null) {
                        document.querySelectorAll(".inputTeamOpt").forEach((x) => {
                            ...
                            const obj = {
                                "deptNo" : x.parentElement.parentElement.previousElementSibling.children[0].value,
                                "teamNm" : x.text
                            }

                            fetch("admin/addInBulk/checkTeamNm", {
                                method : 'post',
                                headers : {"Content-Type" : "application/json"},
                                body : JSON.stringify(obj)
                            })
                            ...
                            .then(count => {
                                if(count > 0) {
                                    x.parentElement.style.borderColor = 'red';
                                    console.log('test') 
                                } else {
                                    console.log(count)
                                }
