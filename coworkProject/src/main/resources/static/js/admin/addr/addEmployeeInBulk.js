                    ...
                    if(check4) {
                        result.forEach((i, index1) => {
                            ...
                            if(positionList != null) {
                                const inputpositionOpt = document.querySelectorAll(".inputpositionOpt");
                                for(let x = inputpositionOpt.length - 1; x >= 0; x--) {
                                    let check8 = false;
                                    if(inputpositionOpt[x].innerText.length > 0) {
                                        positionList.forEach((i) => {
                                            if(i.positionNm == inputpositionOpt[x].text) {
                                                inputpositionOpt[x].parentElement.value = i.positionNo;
                                                check8 = true;
                                                return;
                                            }
                                        })
                                        if(!check8) {
                                            inputpositionOpt[x].parentElement.style.border = '1px solid red';
                                        }
                                    }
                                }
                            }
